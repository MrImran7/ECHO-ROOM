...
void main() {
  ...
  Future<void> finishPattern(WidgetTester tester, Future<void> pattern) async {
    await tester.pump();
    await tester.pump(
      const Duration(milliseconds: GameConfig.hapticPatternGapMilliseconds + 1),
    );
    await pattern;
  }
  ...
  testWidgets(
    'Android first wrong is two beats; rapid repeat is suppressed; correct wins',
    (tester) async {
      ...
      expect(calls, [
        null,
        null,
      ]); // Wrong first beat, then correct; no stale beat.
      calls.clear();
  ...
  testWidgets(
    'Android disabled blocks every cue and probes, including queued beats',
    (tester) async {
      final service = HapticsService(platform: TargetPlatform.android);
      addTearDown(service.dispose);
      ...
      for (final probe in HapticProbe.values) {
        await service.debugProbe(probe);
      }
  ...
  testWidgets(
    'repeated suspend/resume restores first feedback without stale patterns',
    (tester) async {
      final service = HapticsService(platform: TargetPlatform.android);
      for (var i = 0; i < 3; i++) {
  ...
  testWidgets(
    'raw debug probes request separate API types without Android remapping',
    (tester) async {
      final service = HapticsService(platform: TargetPlatform.android);
      addTearDown(service.dispose);
      for (final probe in HapticProbe.values) {
  ...
  testWidgets('iOS keeps semantic feedback and correct uses medium impact', (
    tester,
  ) async {
  ...
  testWidgets(
    'Android platform failure is contained and future dispatch remains available',
    (tester) async {
      final service = HapticsService(platform: TargetPlatform.android);
      addTearDown(service.dispose);
      var fail = true;
  ...
  testWidgets(
    'fresh default is on; runtime preference toggles and persistence retain intent',
    (tester) async {
