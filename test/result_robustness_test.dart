...
Future<ProviderContainer> showResult(
  ...
  unawaited(
    navigator.currentState!.push<void>(
      MaterialPageRoute(
        builder: (_) => ResultScreen(session: s, completion: done),
      ),
    ),
  );
...
void main() {
  testWidgets(
    'Level 20 persists completion and shows story, milestones and all awards without Level 21',
    (tester) async {
  ...
  for (final level in [1, 20]) {
    testWidgets(
      'result $level admits one navigation while storage is pending',
      (tester) async {
        final s = session(level: level);
        win(s);
        final done = completeSession(
          Progress().patch({'highestLevel': level}),
          s,
          DateTime(2026, 9, 10),
          20,
        );
        ...
        expect(
          container.read(sessionProvider.notifier).game!.level.levelId,
          level == 20 ? 20 : 2,
        );
  ...
  for (final stars in [0, 1, 3]) {
    testWidgets(
      '$stars stars and long scores fit a narrow phone with sound and haptics off',
      (tester) async {
        final s = session();
        if (stars == 0) {
          reachAnswer(s);
          s.advance(s.remaining + 2);
        } else {
          win(s);
        }
        ...
        expect(
          find.text(stars == 0 ? 'ROOM UNRESOLVED' : 'ROOM COMPLETE'),
          findsOneWidget,
        );
  ...
  testWidgets('rapid settings toggles preserve each other and survive reload', (
    tester,
  ) async {
    final repo = MemoryProgressRepository();
    final s = session();
    ...
    for (final title in ['Music', 'Sound effects', 'Haptics']) {
      tiles.firstWhere((t) => (t.title as Text).data == title).onChanged!(
        false,
      );
    }
    await tester.pumpAndSettle();
