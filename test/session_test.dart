...
void main() {
  test('full deterministic observation-to-blackout-to-answer flow', () {
    final s = session();
    s.advance(GameConfig.introSeconds);
    expect(s.phase, GamePhase.countdown);
    s.advance(GameConfig.countdownSeconds);
    expect(s.phase, GamePhase.observing);
    s.advance(s.level.observationDuration);
    expect(s.phase, GamePhase.flicker);
    s.advance(GameConfig.flickerSeconds);
  ...
  test('wrong tap uses an attempt; rapid second tap does not', () {
    final s = session();
    reachAnswer(s);
    expect(s.tap(.31, .23), TapResult.wrong);
    expect(s.mistakes, 1);
  ...
  test('empty-space tap is harmless', () {
    final s = session();
    reachAnswer(s);
    expect(s.tap(.99, .01), TapResult.ignored);
    expect(s.mistakes, 0);
  });
  test(
    'background pause cannot lose time; restore requires explicit resume',
    () {
      final s = session();
      reachAnswer(s);
      s.advance(.8);
      ...
      s.advance(1000);
      expect(s.answerElapsed, time);
      expect(s.phase, GamePhase.paused);
      final restored = session()..restore(s.toJson());
      expect(restored.paused, true);
      expect(restored.answerElapsed, time);
      restored.advance(20);
      expect(restored.phase, GamePhase.paused);
      restored.resume();
      restored.advance(10);
      expect(restored.phase, GamePhase.lost);
    },
  );
  test(
    'a long frame carries over phase boundaries without extending timers',
    () {
      final s = session();
      s.advance(100);
  ...
  test('all 20 content levels have a visible change and a hittable target', () {
    ...
    catalog.validate();
    expect(catalog.levels.length, 20);
    for (final level in catalog.levels) {
      final s = session(level: level.levelId);
      expect(
        s.original.objects.map((o) => o.toJson()).toList(),
        isNot(s.changed.objects.map((o) => o.toJson()).toList()),
      );
      win(s);
      expect(s.phase, GamePhase.won, reason: 'Level ${level.levelId}');
    }
  });
    ...
    for (final id in [2, 3, 9]) {
      ...
      for (final target in s.level.targets) {
        for (final o in [s.original.object(target), s.changed.object(target)]) {
          expect(
            s.level.targets.contains(
              detectObject(s.original, s.changed, s.level.targets, o.x, o.y),
            ),
            true,
          );
        }
  ...
  test(
    'unknown change types fail clearly and registered types extend the engine',
    () {
  ...
  test('paid hint highlight and input cooldown survive pause and restore', () {
    ...
    expect(restored.hintRemaining, s.hintRemaining);
    restored.resume();
    expect(restored.tap(.31, .23), TapResult.ignored);
    restored.advance(.4);
    expect(restored.tap(.31, .23), TapResult.wrong);
  });
  test('older final-reveal checkpoints remain useful after restoring', () {
    ...
    expect(restored.hints, 3);
    expect(restored.hintRemaining, greaterThan(0));
  });
  test(
    'damaged checkpoint is rejected without partially changing the session',
    () {
      ...
      for (final patch in <Json>[
        ...
        {'mistakes': -1},
        {'phaseElapsed': -2},
        {'answerElapsed': double.infinity},
        {'levelId': 2},
        {'phase': 'unknown'},
        {'lastTapped': 'missing-object'},
      ]) {
  ...
  test('loading stays inert and readiness survives backgrounding', () {
    ...
    expect(s.phase, GamePhase.paused);
    s.resume();
    expect(s.phase, GamePhase.intro);
  });
  test('timeout reveals the answer before becoming terminal', () {
    ...
    s.resume();
    s.advance(GameConfig.timeoutRevealSeconds);
    expect(s.phase, GamePhase.lost);
    expect(s.lossReason, LossReason.timeout);
  });
  test('correct feedback has fixed duration and freezes response time', () {
    ...
    final elapsed = s.answerElapsed;
    s.advance(GameConfig.correctFeedbackSeconds / 2);
    expect(s.finished, false);
  ...
  test('malformed transform geometry cannot enter a room state', () {
    final s = session();
    expect(
      () => ChangeRegistry().apply(s.original, const [
  ...
  test(
    'overlap resolution respects foreground objects without answer priority',
    () {
      const back = RoomObject(
        id: 'back',
        art: 'book',
        x: .5,
        y: .5,
      ...
      final changed = RoomState([
        back.patch({'visible': false}),
        visible,
      ]);
