...
void main() {
  test('selection depends only on local calendar date and is always in range', () {
    const daily = LocalDailyChallengeSource();
    expect(daily.levelFor(DateTime(2026, 9, 9, 0), 20), daily.levelFor(DateTime(2026, 9, 9, 23, 59), 20));
    final selected = <int>{};
    for (var i = 0; i < 365; i++) {
      final id = daily.levelFor(DateTime(2026, 1, 1 + i), 20);
      expect(id, inInclusiveRange(1, 20)); selected.add(id);
    }
  ...
  test('daily streak increments on consecutive dates and resets after a gap', () {
    var p = Progress();
    for (final day in [9, 10, 12]) {
      final s = session(daily: dateKey(DateTime(2026, 9, day)), runId: '$day'); win(s);
      p = completeSession(p, s, DateTime(2026, 9, day), 20).progress;
      expect(p.dailyStreak, day == 10 ? 2 : 1);
    }
    expect(p.bestDailyStreak, 2); expect(p.levels, isEmpty); expect(p.highestLevel, 1);
  ...
  test('daily failure records attempt without spending chapter lives', () {
    ...
    expect(p.daily['2026-09-09']!.status, 'lost'); expect(p.lives, 5);
  });
}
