...
void main() {
  ...
  test('milestone multipliers apply at exact boundaries', () {
    expect([0, 2, 3, 4, 5, 9, 10, 20].map(streakMultiplier), [
      1,
      1,
      1.2,
      1.2,
  ...
  test('star thresholds account for time, mistakes and hints', () {
    int stars(double time, int misses, int hints) => calculateStars(
      won: true,
      elapsed: time,
      limit: 10,
      mistakes: misses,
    ...
    expect(stars(5, 0, 0), 3);
    expect(stars(5.01, 0, 0), 2);
    expect(stars(1, 1, 0), 2);
    expect(stars(1, 0, 1), 2);
    expect(stars(8.5, 1, 1), 2);
    expect(stars(8.51, 0, 0), 1);
