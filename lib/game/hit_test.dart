...
class HitTester {
  HitTester(RoomState before, RoomState after, Set<String> targets)
    : regions = List.unmodifiable([
        for (final o in after.objects)
          if (o.visible) HitRegion(o),
        for (final id in targets)
          if (before.object(id).visible &&
              (!after.object(id).visible ||
                  before.object(id).x != after.object(id).x ||
                  before.object(id).y != after.object(id).y))
  ...
  String? resolve(double x, double y, {double minimumPadding = .025}) {
    ...
    for (final region in regions) {
      ...
      if (best == null ||
          ...
          (o.z == best.object.z &&
              ((exact && !bestExact) ||
                  (exact == bestExact && distance < bestDistance)))) {
        best = region;
        bestExact = exact;
...
String? detectObject(
  ...
  double x,
  double y, {
  double minimumPadding = .025,
}) => HitTester(
  before,
  after,
  targets,
).resolve(x, y, minimumPadding: minimumPadding);
