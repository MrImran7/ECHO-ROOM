...
class RoomArt {
  ...
  void rect(Canvas c, double x, double y, double w, double h, Color color, [double r = 0]) {
    _paint..color = color..style = PaintingStyle.fill..shader = null;
    c.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(x, y, w, h), Radius.circular(r)), _paint);
  }
  void oval(Canvas c, double x, double y, double w, double h, Color color) {
    _paint..color = color..style = PaintingStyle.fill..shader = null;
    c.drawOval(Rect.fromLTWH(x, y, w, h), _paint);
  }
  void line(Canvas c, double x, double y, double x2, double y2, Color color, [double width = 2]) {
    _paint..color = color..strokeWidth = width..style = PaintingStyle.stroke..shader = null;
  ...
  void label(Canvas c, String text, double x, double y, double size, Color color) {
    final p = _labels.putIfAbsent('$text/$size/$color', () => TextPainter(
  ...
  void background(Canvas c) {
    ...
    _paint..style = PaintingStyle.fill..color = Colors.white
      ..shader = const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight,
        colors: [Color(0xff64766b), Color(0xff354c45)]).createShader(wall);
    c.drawRect(wall, _paint); _paint.shader = null;
    ...
    for (var i = 0; i < 5; i++) {
      _paint..color = const Color(0x40746350)..style = PaintingStyle.stroke..strokeWidth = 1;
      c.drawOval(Rect.fromLTWH(64 + i * 5.0, 329 + i * 3.0, 271 - i * 10.0, 78 - i * 6.0), _paint);
    }
  }

  void object(Canvas c, RoomObject o, {double pulse = 1, double shake = 0}) {
    if (!o.visible) return;
    c.save(); c.translate(o.x * 400 + shake, o.y * 440);
    c.rotate(o.rotation); c.scale(o.width * 4 * o.scale * pulse, o.height * 4.4 * o.scale * pulse);
    c.translate(-50, -50);
    final asset = o.asset == null ? null : images[o.asset];
    if (asset != null) {
      c.drawImageRect(asset, Rect.fromLTWH(0, 0, asset.width.toDouble(), asset.height.toDouble()),
        const Rect.fromLTWH(0, 0, 100, 100), Paint());
    } else { drawObject(c, o); }
    c.restore();
  }
  ...
  void drawObject(Canvas c, RoomObject o) {
    ...
    switch (o.art) {
      case 'sofa':
        ...
        rect(c, 10, 78, 5, 20, ink, 1); rect(c, 85, 78, 5, 20, ink, 1);
      ...
      case 'lamp':
        ...
        polygon(c, const [Offset(24, 1), Offset(76, 1), Offset(97, 32), Offset(3, 32)], cream);
      ...
      case 'plant':
        for (var i = 0; i < 6; i++) {
          ...
          oval(c, x - 4, y, 25, 23, i.isEven ? const Color(0xff779161) : const Color(0xff4c724e));
        }
        polygon(c, [const Offset(22, 62), const Offset(78, 62), const Offset(69, 99), const Offset(31, 99)], color);
      ...
      case 'window':
        ...
        for (var i = 0; i < 6; i++) {
          rect(c, 7 + i * 14.0, 62 - (i % 3) * 9.0, 12, 32 + (i % 3) * 9.0, const Color(0xff172f39));
      ...
      case 'painting':
        ...
        oval(c, o.variant == 'moon' ? 45 : 21, 20, 26, 26,
          o.variant == 'moon' ? const Color(0xffeee5c7) : const Color(0xffc89062));
        polygon(c, [const Offset(11, 85), const Offset(11, 68), Offset(o.variant == 'inverted' ? 70 : 34, 42),
          const Offset(85, 72), const Offset(85, 85)], const Color(0xff35554c));
        polygon(c, const [Offset(11, 85), Offset(62, 50), Offset(85, 65), Offset(85, 85)], const Color(0xff536b57));
        break;
      case 'clock':
        ...
        for (var i = 0; i < 12; i++) {
          final a = i * math.pi / 6;
          line(c, 48 + math.sin(a) * 33, 48 - math.cos(a) * 33,
            48 + math.sin(a) * 36, 48 - math.cos(a) * 36, ink, 1.6);
        }
        line(c, 48, 48, 48, 18, ink, 3);
        line(c, 48, 48, o.variant == 'late' ? 23 : 68, o.variant == 'late' ? 57 : 40, ink, 4);
        if (o.variant == 'impossible') line(c, 48, 48, 32, 23, const Color(0xffa64b38), 3);
      ...
      case 'book':
        ...
        break;
      case 'cup':
        oval(c, 60, 22, 38, 44, color); oval(c, 70, 31, 18, 23, const Color(0xffceaa78));
        rect(c, 5, 14, 65, 73, color, 14);
        oval(c, 5, 5, 65, 22, cream); oval(c, 13, 10, 49, 12, const Color(0xff533b28));
        if (o.variant == 'striped') { rect(c, 8, 40, 59, 9, cream); rect(c, 8, 62, 59, 9, cream); }
        break;
      case 'chair':
        rect(c, 10, 2, 80, 50, color, 8);
        rect(c, 15, 40, 6, 58, ink); rect(c, 79, 40, 6, 58, ink);
      ...
      case 'mirror':
        ...
        polygon(c, const [Offset(20, 75), Offset(77, 20), Offset(86, 38), Offset(33, 90)], const Color(0x30d8e6d8));
        rect(c, 18, 69, 64, 5, const Color(0xffc1a17c));
        oval(c, o.variant == 'other' ? 56 : 26, 44, 17, 25, const Color(0xffdcc598));
        if (o.variant == 'visitor') {
          oval(c, 58, 33, 16, 16, const Color(0xff314d49)); rect(c, 55, 47, 23, 22, const Color(0xff314d49), 8);
        }
        break;
      case 'shadow':
        polygon(c, o.variant == 'left' ? const [Offset(85, 10), Offset(5, 68), Offset(38, 90), Offset(95, 30)]
          : const [Offset(5, 10), Offset(95, 68), Offset(62, 90), Offset(5, 30)], const Color(0x50393025));
        break;
      case 'note':
        ...
        break;
      case 'vase':
        rect(c, 31, 0, 38, 35, color, 5); oval(c, 11, 22, 78, 77, color);
      ...
      case 'key':
        ...
        break;
      case 'light':
        oval(c, 0, 0, 100, 100, o.variant == 'dim' ? const Color(0x0dfcdb87) : const Color(0x40fcdb87));
        break;
      default:
