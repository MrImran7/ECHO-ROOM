...
class GameCatalog {
  ...
  static Future<GameCatalog> load() async {
    final data = jsonDecode(
      await rootBundle.loadString('assets/rooms/apartment.json'),
    ) as Json;
    final rooms = (data['rooms'] as List<dynamic>).map(
      (j) => Room.fromJson(j as Json),
    );
    final catalog = GameCatalog(
      rooms: {for (final r in rooms) r.id: r},
      chapters: (data['chapters'] as List<dynamic>? ?? [])
          .map((j) => ChapterDefinition.fromJson(j as Json))
          .toList(),
      levels: (data['levels'] as List<dynamic>)
          .map((j) => LevelDefinition.fromJson(j as Json))
          .toList(),
    );
    catalog.validate();
    return catalog;
