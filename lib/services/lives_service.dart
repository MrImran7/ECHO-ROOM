...
class LivesService {
  const LivesService({this.maximum = GameConfig.maxLives,
  ...
  Progress refresh(Progress p, DateTime now) {
    if (p.lives >= maximum) return p.patch({'lives': maximum, 'lifeAnchor': null});
    ...
    return p.patch({'lives': lives, 'lifeAnchor': lives == maximum ? null :
      anchor.add(Duration(seconds: count * math.max(1, regeneration.inSeconds))).toIso8601String()});
  }
  Progress consume(Progress p, DateTime now) {
    final current = refresh(p, now);
    return current.patch({'lives': math.max(0, current.lives - 1),
  ...
  Duration? untilNext(Progress p, DateTime now) {
    ...
    return Duration(seconds: math.max(0, regeneration.inSeconds - now.difference(p.lifeAnchor!).inSeconds));
  }
}
