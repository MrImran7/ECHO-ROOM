...
enum SoundCue { menu, countdown, flicker, correct, wrong, streak, complete, mystery }
...
class LocalAudioService implements AudioService {
  ...
  Future<void> _safe(Future<void> Function() action) async {
    if (_disposed) return;
    try { await action(); } catch (e) { if (kDebugMode) debugPrint('Audio unavailable: $e'); }
  }
  @override
  void cue(SoundCue cue) {
  ...
  @override
  void suspend() {
    _suspended = true; _syncMusic();
    for (final p in _players.values) { unawaited(_safe(p.pause)); }
  }
  @override
  void resume() { _suspended = false; _syncMusic(); }
  @override
...
class HapticsService {
  bool enabled = true;
  Future<void> _safe(Future<void> Function() action) async {
    try { await action(); } catch (e) { if (kDebugMode) debugPrint('Haptics unavailable: $e'); }
  }
  Future<void> correct() async { if (enabled) await _safe(HapticFeedback.lightImpact); }
  Future<void> wrong() async { if (enabled) await _safe(HapticFeedback.heavyImpact); }
