...
class AchievementDefinition {
  ...
  String progressLabel(Progress p, int chapterSize) => switch (metric) {
    AchievementMetric.completedLevels =>
      '${p.roomsCompleted(chapterSize)} / $target rooms',
    AchievementMetric.perfectLevels =>
      '${p.perfectLevels.length} / $target rooms',
    AchievementMetric.fastestFind =>
      p.bestResponseTime == null
          ? 'Find a change to set a best time'
          : 'Best ${p.bestResponseTime!.toStringAsFixed(2)}s',
    AchievementMetric.unassistedLevels =>
      '${p.unassistedLevels.length} / $target rooms',
    AchievementMetric.chapterComplete =>
      '${p.roomsCompleted(chapterSize)} / $chapterSize rooms',
    AchievementMetric.threeStarLevels =>
      '${p.levels.values.where((r) => r.stars == 3).length} / $target rooms',
    AchievementMetric.totalStars =>
      '${p.totalStars(chapterSize)} / $target stars',
  ...
  bool satisfied(Progress p, int chapterSize) => switch (metric) {
    ...
    AchievementMetric.threeStarLevels =>
      p.levels.entries
              .where(
                (e) => e.key >= 1 && e.key <= chapterSize && e.value.stars == 3,
              )
              .length >=
          target,
...
const achievements = [
  ...
  AchievementDefinition(
    'perfectionist',
    'PERFECTIONIST',
    'Earn 3 stars on 10 distinct rooms.',
    AchievementMetric.threeStarLevels,
    10,
  ),
  AchievementDefinition(
  ...
  AchievementDefinition(
    'streak_master',
    'STREAK MASTER',
    'Solve 10 rooms in a row without failing.',
