...
class RoomObject {
  ...
  factory RoomObject.fromJson(Json j) => RoomObject(
    ...
    rotation: number(j['rotation']),
    scale: number(j['scale'], 1),
    visible: j['visible'] as bool? ?? true,
    z: j['z'] as int? ?? 0,
    color: j['color'] as int? ?? 0xffbd7355,
    text: j['text'] as String? ?? '',
    variant: j['variant'] as String? ?? 'default',
    asset: j['asset'] as String?,
    hitPadding: number(j['hitPadding'], .015),
  );
  Json toJson() => {
    'id': id,
    'art': art,
  ...
  RoomObject patch(Json patch) =>
      RoomObject.fromJson({...toJson(), ...patch, 'id': id});
...
class Room {
  Room({
    required this.id,
    required this.name,
    required List<RoomObject> objects,
  ...
  factory Room.fromJson(Json j) => Room(
    ...
    objects: (j['objects'] as List<dynamic>)
        .map((e) => RoomObject.fromJson(e as Json))
        .toList(),
  );
}
...
class Change {
  ...
  final Json values;
  factory Change.fromJson(Json j) => Change(
    type: j['type'] as String,
    objectId: j['objectId'] as String,
    values: j['values'] as Json? ?? {},
  );
...
class ChangeRegistry {
  ...
  RoomState apply(RoomState original, List<Change> changes) {
    ...
    for (final change in changes) {
      ...
      if (index < 0 || transform == null)
        throw FormatException('Invalid change ${change.toJson()}');
...
class LevelDefinition {
  ...
  factory LevelDefinition.fromJson(Json j) => LevelDefinition(
    levelId: j['levelId'] as int,
    roomId: j['roomId'] as String,
    difficulty: j['difficulty'] as String,
    changes: (j['changes'] as List<dynamic>)
        .map((e) => Change.fromJson(e as Json))
        .toList(),
    hints: (j['hints'] as List<dynamic>).cast<String>(),
    observationDuration: number(j['observationDuration'], 7),
    answerDuration: number(j['answerDuration'], 5),
    attempts: j['attempts'] as int? ?? 2,
